function ComputeSMA(time_s, window_size)
{
     ...
     for (let i = 0; i <= time_s.length - window_size; i++)
     {
          ...
          for (let k = i; k < t && k <= time_s.length; k++)
               curr_avg += time_s[k]['confirmed'] / window_size;
...
function GenerateDataset(size)
{
    ...
    for (let i = 0; i < size; i++, curr_time+=time_diff) {
       ...
       dataset.push({ id: i + 1, confirmed: (Math.floor(Math.random() * 10) + 5) + Math.random(),
	  confirmed: curr_dt.getFullYear() + "-" + ((curr_dt.getMonth() > 9) ? curr_dt.getMonth() : ("0" + curr_dt.getMonth())) + "-" +
